...
1. Mass transfer plays a central role in both humidification and dehumidification processes. Moisture is introduced into the air stream by bubbling air through liquid water, promoting evaporation, while a parallel stream passes through a desiccant to remove water vapor. The final humidity within the chamber is determined by the balance between these two opposing mass transfer processes.

2. The system uses relative humidity as the primary measured and controlled variable. Relative humidity reflects the ratio of water vapor present in the air to the maximum amount it can hold at a given temperature. This relationship allows the sensor to quantify the moisture level inside the chamber and enables direct comparison to the desired setpoint.

3. The system is also governed by vapor–liquid equilibrium (VLE), which defines the maximum amount of water vapor that air can hold at a given temperature. Since the system operates at ambient conditions, the saturation limit constrains the achievable humidity of the humidified air stream.
